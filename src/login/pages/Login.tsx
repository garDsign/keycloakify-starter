        ...
        <Template
            ...
            headerNode={
                <div className="flex items-center justify-between w-full">
                    ...
                    {/* Language Switcher */}
                    {realm.internationalizationEnabled && (kcContext.locale?.supported.length ?? 0) > 1 && (
                        <select
                            value={kcContext.locale?.currentLanguageTag}
                            onChange={(e) => {
                                const newLocale = e.target.value;
                                // Use loginUrl and add kc_locale parameter, this prevents carrying over error messages
                                const baseUrl = url.loginUrl.split('?')[0];
                                const params = new URLSearchParams(url.loginUrl.split('?')[1] || '');
                                params.delete('error');
                                params.delete('error_description');
                                params.set('kc_locale', newLocale);
                                window.location.href = `${baseUrl}?${params.toString()}`;
                            }}
                            className="px-2 py-1 rounded-md border text-xs cursor-pointer font-medium appearance-none w-14"
                            style={{
                                borderColor: 'var(--color-green-dark)',
                                backgroundColor: 'transparent',
                                color: 'var(--color-text-primary)',
                                backgroundImage: `url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%23162C21' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e")`,
                        ...
                        >
                            {kcContext.locale?.supported.map((locale) => (
                                <option key={locale.languageTag} value={locale.languageTag}>
                                    {locale.languageTag.split('-')[0].toUpperCase()}
                                </option>
                            ))}
                        </select>
                    )}
                            ...
                            {!usernameHidden && (
                                <div className="form-group mb-5">
                                    <label htmlFor="username" className="block text-sm font-medium mb-2" style={{color: 'var(--color-text-primary)'}}>
                                        {realm.loginWithEmailAllowed && !realm.registrationEmailAsUsername ? msg("usernameOrEmail") : msg("email")}
                                    </label>
                                    <input
                                        tabIndex={2}
                                        id="username"
                                ...
                                <div className="flex items-center justify-between mb-3">
                                    {realm.rememberMe && !usernameHidden ? (
                                        <div className="flex items-baseline gap-2">
                                            ...
                                            />
                                            <label htmlFor="rememberMe" className="text-sm font-normal remember-label relative -translate-y-[3px]" style={{color: 'var(--color-text-primary)'}}>
                                                {msg("rememberMe")}
                                            </label>
                                        </div>
                                    ) : <div />}
                                ...
                                <>
                                    <div className="relative my-6">
                                        ...
                                        <div className="relative flex justify-center text-sm">
                                            <span className="px-2" style={{backgroundColor: 'var(--color-beige-dark)', color: 'var(--color-text-tertiary)'}}>or</span>
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        {social.providers.map((p) => (
                                            ...
                                            >
                                                ...
                                                <span className="text-lg font-medium" style={{color: 'var(--color-text-primary)'}}>{p.displayName}</span>
                                            </a>
                                        ))}
                                    </div>
                                </>
